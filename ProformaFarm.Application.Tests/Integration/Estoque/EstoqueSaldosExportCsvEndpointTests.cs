...
public sealed class EstoqueSaldosExportCsvEndpointTests : IClassFixture<CustomWebApplicationFactory>
{
    private readonly CustomWebApplicationFactory _factory;

    public EstoqueSaldosExportCsvEndpointTests(CustomWebApplicationFactory factory)
    {
        _factory = factory;
    }
    ...
    public async Task Sem_token_deve_retornar_401()
    {
        ...
        var response = await client.GetAsync("/api/estoque/saldos/exportar-csv");
    ...
    public async Task Com_token_deve_retornar_csv_com_cabecalho_e_dados()
    {
        ...
        var response = await client.GetAsync($"/api/estoque/saldos/exportar-csv?idOrganizacao={setup.IdOrganizacao}&idProduto={setup.IdProduto}&limite=100");
        ...
        Assert.Contains("saldos", string.Join(",", response.Headers.GetValues("X-Export-Resource")));

        var csv = await response.Content.ReadAsStringAsync();
        Assert.Contains("IdEstoque,IdOrganizacao", csv);
